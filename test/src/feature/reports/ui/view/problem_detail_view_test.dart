...
void main() {
  group('ProblemDetailView', () {
    testWidgets('제출 성공 후 자동으로 결과 탭으로 이동할 때는 이력을 먼저 조회하지 않는다', (tester) async {
    ...
    testWidgets('사용자가 결과 탭을 직접 선택하면 기존처럼 제출 이력을 조회한다', (tester) async {
      await tester.binding.setSurfaceSize(const Size(1440, 2000));
      addTearDown(() => tester.binding.setSurfaceSize(null));
    ...
    testWidgets('채점 스트림이 끝나기 전에는 결과 탭을 다시 눌러도 이력을 조회하지 않는다', (tester) async {
      await tester.binding.setSurfaceSize(const Size(1440, 2000));
      addTearDown(() => tester.binding.setSurfaceSize(null));
