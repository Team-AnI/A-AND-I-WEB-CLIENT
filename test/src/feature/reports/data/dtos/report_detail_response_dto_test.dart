...
void main() {
  group('ReportDetailResponseDto', () {
    ...
    test('입력이 비어 있는 예제 테스트 케이스도 파싱한다', () {
      final dto = ReportDetailResponseDto.fromJson({
        'success': true,
        'data': {
          'assignmentId': 'assignment-1',
      ...
      expect(report, isNotNull);
      expect(report!.exampleIo, hasLength(1));
      expect(report.exampleIo.first.input, isEmpty);
      expect(report.exampleIo.first.output, '출력만 있는 케이스');
    });

    test('출력이 비어 있는 예제 테스트 케이스도 파싱한다', () {
      final dto = ReportDetailResponseDto.fromJson({
        'success': true,
        'data': {
          'assignmentId': 'assignment-1',
      ...
      expect(report, isNotNull);
      expect(report!.exampleIo, hasLength(1));
      expect(report.exampleIo.first.input, '입력만 있는 케이스');
      expect(report.exampleIo.first.output, isEmpty);
    });
  });
}
